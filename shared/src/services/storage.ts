...
export function setProjectsDir(dir: string) {
  PROJECTS_DIR = dir;
}
...
export async function initializeStorage(dataDir?: string) {
  if (dataDir) {
...
export async function updateEntry(projectId: string, entryId: string, data: FeedIngestResult): Promise<Entry> {
  const entryPath = path.join(PROJECTS_DIR, projectId, 'entries', `${entryId}.json`);

  const existing = await getEntry(projectId, entryId);
  if (!existing) {
  ...
  await fs.writeFile(entryPath, JSON.stringify(updated, null, 2));
  return updated;
}
